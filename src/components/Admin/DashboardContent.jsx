...
import { faCheck, faWarning, faUsers, faClose, faDashboard, faTasks, faList, faAdd, faServer } from '@fortawesome/free-solid-svg-icons';
...
export default function DashboardContent() {
    ...
    const [totalLeaveRequests, setTotalLeaveRequests] = useState();
    const [totalSystemReports, setTotalSystemReports] = useState();
    ...
    async function populateDashboard(){
        const res = await fetch('http://localhost:1337/api/dashboard-content');
        const data = await res.json();
        if(data.status=='ok'){
            setTotalUsers(data.totalUsers);
            setTotalLeaveRequests(data.totalLeaveRequests)
            setTotalSystemReports(data.totalSystemReports);
            
            const responseData = data.data.splice(0,5)
            const tempData = responseData.map(user=>(
  ...
  return (
    <div className="col-lg-9 ms-3 p-4">
        <div className='row'>
            <div className='col col-lg-3 d-inline bg-white rounded m-3 pe-3 p-2 shadow-lg text-center border'><p className='mb-1 text-dark fs-5'>Total Students</p> <p className='text-primary fs-2 fw-bold'><FontAwesomeIcon icon={faUsers} className='me-2' />{totalUsers}</p></div>
            <div className='col col-lg-3 d-inline bg-white rounded m-3 pe-3 p-2 shadow-lg text-center border'><p className='mb-1 text-dark fs-5'>Active Leave Requests </p> <p className='text-warning fs-2 fw-bold'><FontAwesomeIcon icon={faWarning} className='me-2' />{totalLeaveRequests}</p></div>
            <div className='col col-lg-3 d-inline bg-white rounded m-3 pe-3 p-2 shadow-lg text-center border'><p className='mb-1 text-dark fs-5'>Total System Reports </p> <p className='text-info fs-2 fw-bold'><FontAwesomeIcon icon={faServer} className='me-2' />{totalSystemReports}</p></div>
        ...
        <h4 className='mb-3'>Student Records</h4>
            ...
            <Button variant='dark' className=' fs-6 mb-4' onClick={()=>navigate('../view-records')}>View All <FontAwesomeIcon icon={faList} className='me-2' /></Button>
            </div>
            ...
            <Modal.Body>
                <Table striped bordered hover className='text-center'>
                    ...
                    <tbody>
                    {
                        ...
                        <tr><td colSpan='6'><Alert variant='danger' className='w-100'>No Data exists</Alert></td></tr>
                    }
